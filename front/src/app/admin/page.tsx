...
import { Users, FileCheck, AlertCircle, TrendingUp, Search, Filter } from 'lucide-react';
                    ...
                    <div className="mb-12">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <div className="flex gap-3">
                                <ThemeControls />
                                <button className={`px-4 py-2 border-2 rounded-xl transition-all hover:shadow-md flex items-center gap-2 ${isDark ? 'bg-slate-800 border-slate-700 text-slate-300 hover:border-blue-500' : 'bg-white border-slate-200 text-slate-700 hover:border-blue-300'}`}>
                                ...
                                <tbody className={`divide-y ${isDark ? 'divide-slate-700' : 'divide-slate-100'}`}>
                                    {filteredSubmissions.map((submission) => (
                                        <tr key={submission.id} className={`transition-colors ${isDark ? 'hover:bg-slate-700/50' : 'hover:bg-blue-50/50'}`}>
                                            <td className={`px-8 py-4 font-bold ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>{submission.user.username}</td>
